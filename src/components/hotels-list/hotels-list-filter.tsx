...
export default function HotelsFilter(): React.ReactNode {
  ...
  const [openDepartCalendar, setOpenDateCalendar] = useState<boolean>(false);
    ...
    <div className="hidden lg:flex flex-col w-full">
      <div className="rounded-md shadow-sm w-full mt-8">
        <Center axe={Axes.y}>
          ...
          >
            <div className="flex flex-row w-full xl:h-full xl:max-w-[230px] lg:w-[320px] h-12 border border-transparent lg:rounded-none rounded-2xl lg:mx-0 items-center lg:gap-2 gap-4 px-2 xl:pl-4">
              <Localisation className="absolute top-4 size-6 left-5" />

              <input
                type="text"
                id="first_name"
                className="text-[19px] bg-transparent w-full text-black focus:ring-blue-500 focus:border-blue-500 block lg:pl-10 pl-24 p-2"
                placeholder="Votre destination"
                {...register("location")}
              />
            </div>
            <div className="flex z-50 xl:max-w-[450px] lg:w-[650px] xl:text-[0.8rem] xl:min-w-[350px] sm:flex-row flex-col xl:px-4 xl:h-full sm:h-12 w-full xl:gap-6 sm:gap-8 gap-4 bg-white border-x-2 sm:justify-around xl:justify-center lg:rounded-none rounded-2xl lg:border-x-blue-700 px-4">
